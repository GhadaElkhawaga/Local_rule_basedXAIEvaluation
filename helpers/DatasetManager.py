#to import functions and definitions from external ipynb files
import sys
...
sys.path.append('/content/gdrive/helpers')
x = drive.CreateFile({'id':'1eh5tafzkJvFSmZ9uFJcxjUacPyDPOWM0'})
x.GetContentFile('Definitions.ipynb')
...
class DatasetManager:
    ...
    def read_dataset(self):
        ...
        # to avoid the segmentation fault in linux, with large datasets like the one of HospitalBilling:
        '''BPIC2017_datasets = [os.path.join(logs_dir,'BPIC2017_O_Accepted.csv'), os.path.join(logs_dir,'BPIC2017_O_Cancelled'), os.path.join(logs_dir,'BPIC2017_O_Refused')]
        hos_datasets = [os.path.join(logs_dir,'hospital_billing_1'), os.path.join(logs_dir,'hospital_billing_2')]
        if (Definitions.filename[self.d_name] in BPIC2017_datasets) or (Definitions.filename[self.d_name] in hos_datasets):
           TextFileReaderObject=pd.read_csv(Definitions.filename[self.d_name], sep=';',dtype=dtypes, engine='c', encoding='ISO-8859-1',
                         error_bad_lines=False, chunksize=10**5)
